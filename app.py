...
if data_input:
    try:
        ...
        # --- ORDENAR ESCALONADO POR FECHA DE INICIO MÁS TEMPRANA ---
        ...
        # Formatear Ejes e Identidad Visual
        ax.set_yticks([]) # Ocultar eje Y tradicional
        ax.xaxis.set_major_locator(mdates.MonthLocator())
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%B \'%y'))
        
        # Ajustar colores de los meses arriba
        ax.tick_params(axis='x', colors=config["text_dim"], labelsize=10, pad=10)
        ax.xaxis.tick_top() # Mover los meses arriba de la slide
        
        # Remover bordes feos del gráfico
        for spine in ["left", "right", "bottom", "top"]:
            ax.spines[spine].set_visible(False)
            
        # Añadir línea de división superior
        ax.spines["top"].set_color(config["border_color"])
        ax.spines["top"].set_visible(True)
        ax.spines["top"].set_linewidth(1.5)

        # Títulos integrados dentro de la imagen
        plt.text(0.02, 0.95, proyecto_tag.upper(), transform=fig.transFigure, color=config["accent_primary"], fontsize=11, fontweight='bold', tracking=2)
        plt.text(0.02, 0.89, titulo_slide.upper(), transform=fig.transFigure, color=config["text_main"], fontsize=22, fontweight='bold')
        
        # Footer integrado dentro de la imagen
        plt.text(0.02, 0.04, f"{marca.replace('_', ' ')} / CREATIVE & OPERATIONS", transform=fig.transFigure, color=config["text_main"], fontsize=9, fontweight='bold')
        plt.text(0.98, 0.04, "DIAPOSITIVA AUTOMATIZADA", transform=fig.transFigure, color=config["text_dim"], fontsize=9, ha='right')

        plt.tight_layout()
        plt.subplots_adjust(top=0.78, bottom=0.12, left=0.05, right=0.92)
        ...
        # Mostrar la vista previa de la imagen final generada
        ...
        st.error(f"Error al procesar el Excel. Revisá el formato de las fechas. Detalles: {e}")
